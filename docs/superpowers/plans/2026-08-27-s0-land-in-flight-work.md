...
```sh
set -euo pipefail
S0_STATE=/tmp/subscription-manager-s0-state.env
FIXED_POINT=$(git rev-parse --short HEAD)
printf 'FIXED_POINT=%s\n' "$FIXED_POINT" > "$S0_STATE"
git status --short --untracked-files=all | wc -l
cat "$S0_STATE"
```
...
把固定起点保存到仓库外的 `/tmp/subscription-manager-s0-state.env`；不得把实际 SHA 写回任何已跟踪文件。后续每个需要该值的步骤都显式读取这份记录，并用参数校验拒绝空值，避免空变量让 `git log "$FIXED_POINT"..HEAD` 退化成合法但误导性的范围。任务 2 取得的三个 issue 号也追加到同一份临时记录。
...
S0_STATE=/tmp/subscription-manager-s0-state.env
source "$S0_STATE"
: "${FIXED_POINT:?任务 1 尚未记录 FIXED_POINT}"
printf 'FIXED_POINT=%s\n' "$FIXED_POINT" > "$S0_STATE"
gh issue list --state open --limit 100 --json number,title \
  --jq '.[] | select(.title == "SwiftLint excluded paths miss nested SwiftPM build directories") | "ISSUE_RC1=\(.number)"' >> "$S0_STATE"
gh issue list --state open --limit 100 --json number,title \
  --jq '.[] | select(.title == "Renewal progress is derived in the device time zone, and its VoiceOver label is unlocalized") | "ISSUE_RC2=\(.number)"' >> "$S0_STATE"
gh issue list --state open --limit 100 --json number,title \
  --jq '.[] | select(.title == "Mac library view duplicates the Core table query and sorts with different collation") | "ISSUE_RC3=\(.number)"' >> "$S0_STATE"
test "$(grep -c '^ISSUE_RC[123]=[0-9][0-9]*$' "$S0_STATE")" -eq 3
cat "$S0_STATE"
```

三个号以 `ISSUE_RC1`、`ISSUE_RC2`、`ISSUE_RC3` 保存到仓库外的临时记录；不得写回本计划或其他已跟踪文件。任务 4–7 每次使用前都重新读取并校验它们。
...
按 `production-flow.md` §4，审核对象是从临时记录中的 `FIXED_POINT` 起的完整候选 diff，且必须先给出完整清单。两个审核者会在审阅内容之前驳回任何未归类的清单条目。

- [ ] **步骤 1：生成清单与 diff 材料**

从任务 1 步骤 4 创建的仓库外临时记录读取固定起点。

```sh
source /tmp/subscription-manager-s0-state.env
: "${FIXED_POINT:?任务 1 尚未记录 FIXED_POINT}"
...
if grep -q 'docs/superpowers/' /tmp/s0-tracked.diff /tmp/s0-untracked.diff; then
  echo "错误：候选 diff 混入 docs/superpowers/" >&2
  exit 1
fi
```

预期：`git log` 为空（文档提交就是 `FIXED_POINT` 本身，不在其后）；清单 **26** 条；候选 diff 不含 `docs/superpowers/`；已跟踪 diff 恰好覆盖 **17** 个文件；两个 diff 文件均非空。
...
```sh
set -euo pipefail
swiftlint lint --quiet --reporter csv > /tmp/s0-rc1-swiftlint.csv
awk 'NR > 1 { count++ } END { print count + 0; exit count != 0 }' /tmp/s0-rc1-swiftlint.csv
awk '/\/\.build\// { count++ } END { print count + 0; exit count != 0 }' /tmp/s0-rc1-swiftlint.csv
echo "swiftlint passed: 0 violations and no nested build paths"
swift test --package-path Packages/SubscriptionCore \
  --scratch-path "$HOME/.cache/subscriptionmanager-spm" \
  -Xswiftc -warnings-as-errors 2>&1 | tee /tmp/s0-rc1-core-tests.log
grep -E 'Test run with 253 tests in 12 suites passed' /tmp/s0-rc1-core-tests.log
```
...
swift test --package-path Packages/SubscriptionCore \
  --scratch-path "$HOME/.cache/subscriptionmanager-spm" \
  -Xswiftc -warnings-as-errors 2>&1 | tee /tmp/s0-rc2-core-tests.log
grep -E 'Test run with 262 tests in 14 suites passed' /tmp/s0-rc2-core-tests.log
PYTHONDONTWRITEBYTECODE=1 python3 Scripts/verify_repository.py 2>&1 | \
  tee /tmp/s0-rc2-repository-verifier.log
grep -E '^repository verification passed:' /tmp/s0-rc2-repository-verifier.log
swiftlint lint --quiet --reporter csv > /tmp/s0-rc2-swiftlint.csv
awk 'NR > 1 { count++ } END { print count + 0; exit count != 0 }' /tmp/s0-rc2-swiftlint.csv
...
xcodebuild build -project SubscriptionManager.xcodeproj -scheme SubscriptionManager \
  ...
  2>&1 | tee /tmp/s0-rc2-ios-build.log
grep -E '^\*\* BUILD SUCCEEDED \*\*$' /tmp/s0-rc2-ios-build.log
xcodebuild build -project SubscriptionManager.xcodeproj -scheme SubscriptionManager \
  -destination 'platform=macOS' \
  CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO \
  SWIFT_TREAT_WARNINGS_AS_ERRORS=YES GCC_TREAT_WARNINGS_AS_ERRORS=YES \
  2>&1 | tee /tmp/s0-rc2-macos-build.log
grep -E '^\*\* BUILD SUCCEEDED \*\*$' /tmp/s0-rc2-macos-build.log
```
...
swift test --package-path Packages/SubscriptionCore \
  --scratch-path "$HOME/.cache/subscriptionmanager-spm" \
  -Xswiftc -warnings-as-errors 2>&1 | tee /tmp/s0-full-core-tests.log
grep -E 'Test run with 268 tests in 15 suites passed' /tmp/s0-full-core-tests.log
swiftlint lint --quiet --reporter csv > /tmp/s0-full-swiftlint.csv
awk 'NR > 1 { count++ } END { print count + 0; exit count != 0 }' /tmp/s0-full-swiftlint.csv
echo "swiftlint passed: 0 violations"
PYTHONDONTWRITEBYTECODE=1 python3 -m unittest Scripts.verify_repository_tests 2>&1 | \
  tee /tmp/s0-repository-verifier-tests.log
grep -E '^OK$' /tmp/s0-repository-verifier-tests.log
PYTHONDONTWRITEBYTECODE=1 python3 Scripts/verify_repository.py 2>&1 | \
  tee /tmp/s0-repository-verifier.log
grep -E '^repository verification passed:' /tmp/s0-repository-verifier.log
Scripts/verify_release_logs_tests.sh 2>&1 | tee /tmp/s0-release-log-tests.log
grep -E '^PASS: clean release log passes$' /tmp/s0-release-log-tests.log
swift run --package-path Packages/SubscriptionCore \
  --scratch-path "$HOME/.cache/subscriptionmanager-spm" \
  -Xswiftc -warnings-as-errors CatalogValidator \
  SubscriptionManager/Resources/catalog-v1.json 2>&1 | tee /tmp/s0-catalog-validator.log
grep -E '^valid catalog: schema=1 .*presets=93 offers=190' /tmp/s0-catalog-validator.log
...
for D in 'platform=iOS Simulator,name=iPhone 17 Pro Max,OS=latest' 'platform=macOS' ; do
  xcodebuild test -project SubscriptionManager.xcodeproj -scheme SubscriptionManager \
    ...
    SWIFT_TREAT_WARNINGS_AS_ERRORS=YES GCC_TREAT_WARNINGS_AS_ERRORS=YES \
    2>&1 | tee /tmp/s0-app-tests.log
  grep -E '^\*\* TEST SUCCEEDED \*\*$' /tmp/s0-app-tests.log
done
for D in 'generic/platform=macOS' 'generic/platform=iOS' ; do
  xcodebuild build -project SubscriptionManager.xcodeproj -scheme SubscriptionManager \
    ...
    CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO \
    SWIFT_TREAT_WARNINGS_AS_ERRORS=YES GCC_TREAT_WARNINGS_AS_ERRORS=YES \
    2>&1 | tee /tmp/s0-release-build.log
  grep -E '^\*\* BUILD SUCCEEDED \*\*$' /tmp/s0-release-build.log
done
```
...
test "$(git rev-list --count "$FIXED_POINT"..HEAD)" -eq 3
```
...
读取 `/tmp/subscription-manager-s0-state.env` 并校验 `ISSUE_RC1`、`ISSUE_RC2`、`ISSUE_RC3` 都非空；对三个 issue 号各执行一次 `gh issue comment <n> --body "..."`，正文首行为 `> *This was generated by AI during triage.*`，并记录：该提交 SHA、验收标准逐条结果、任务 4–6 引用的实测命令输出、两轴审核结论与裁决。按 `issue-tracker.md`，issue 在 `artifact_verified` 保持开启，只有 `remote_verified` 之后才关闭。
...
**2. 运行状态扫描。** 无"待定"、"TODO"、"类似任务 N"，也不把实际 SHA 或 issue 号写回已跟踪文件。任务 1 步骤 4 与任务 2 步骤 4 把 `FIXED_POINT`、`ISSUE_RC1`、`ISSUE_RC2`、`ISSUE_RC3` 保存到仓库外的 `/tmp/subscription-manager-s0-state.env`；后续任务每次都显式读取并校验所需值。所有多路径列表均为字面量，不经变量展开，理由见前置事实的 Shell 约束。
